...
describe('onSession login tests', async function () {
        ...
        describe(c.description, function () {
            ...
            it('Guest refresh token login works', async function () {
                ...
                getCurrentBasketRes = await fetch(actions.GET_USER_BASKET, {
                    headers: {
                        cookie: secondGuestShopperSession.cookies
                    }
                });
                ...
                var secondGuestShopperCookiesAsObject =
                ...
                assert.notStrictEqual(
                    ...
                    secondGuestShopperCookiesAsObject[
                        expectedCookieNames.guest
                    ],
                    'The new guest cookie should not be the same as the previous'
                );
            }).timeout(config.timeouts.tests);
            ...
            it('Registered refresh token triggers registered login', async function () {
                ...
                assert.notStrictEqual(
                    ...
                    registeredRefreshCookies[expectedCookieNames.registered],
                    'The new registered cookie should not be the same as the previous'
                );
            }).timeout(config.timeouts.tests);
            ...
            it('Registered user logout works', async function () {
                ...
                var logoutResponseCookies =
                    testHelpers.parseCookies(logoutResponse);
                ...
                var logoutRedirectResponseCookies = testHelpers.cookiesAsObject(
                    testHelpers.parseCookies(logoutRedirectResponse).join(';')
                );

                assert.notExists(
                    logoutRedirectResponseCookies[expectedCookieNames.guest],
                    'Response should delete guest cookie'
                );
                assert.notExists(
                    logoutRedirectResponseCookies[
                        expectedCookieNames.registered
                    ],
                ...
                assert.notExists(
                    logoutRedirectResponseCookies[expectedCookieNames.usid],
    ...
    it('Session is reused when switching between sites', async function () {
        ...
        var combinedCookies = Object.keys(combinedCookiesObj).map(function (
            key
        ) {
