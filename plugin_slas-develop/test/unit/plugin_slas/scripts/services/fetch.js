...
describe('Fetch Tests', function () {
    ...
    it('Fetch gets expected response', function () {
        ...
        var options = {
            ...
            method: 'POST',
            useCredentials: true,
            onCredentials: function (op, credentials) {
                op.credential = credentials.user; // eslint-disable-line no-param-reassign
            },
            timeout: 0
        };
        ...
        expect(serviceSetURL.calledWithMatch(urlWithQueryString)).to.be.ok;
        expect(serviceCall.calledWithMatch(expectedServiceCallParameters)).to.be
            .ok;
    });
